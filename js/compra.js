...
function actualizarTotal() {
  ...
  contenedorTotal.innerHTML = `S/. ${totalCalculado}`;
}

btnAplicarDescuento.addEventListener("click", aplicarDescuento);

function aplicarDescuento() {
  const codigoDescuento = inputCodigoDescuento.value.toLowerCase();
  const totalCalculado = productosCarrito.reduce(
    (acc, producto) => acc + producto.precio * producto.cantidad,
    0
  );
  ...
  if (codigoDescuento === "nuevohl") {
    ...
    contenedorTotal.innerHTML = `S/. ${totalConDescuento}`;
  ...
  fetch("../js/usuarios.json")
    ...
    .then((data) => {
      ...
      if (usuario) {
        mensajeElement.style.display = "block";
        mensajeElement.textContent =
          "Bienvenido de nuevo, te regalamos un 20% de descuento. Ingresa el código OTRAVEZ23";
      } else {
        mensajeElement.style.display = "block";
        mensajeElement.textContent =
          "Bienvenido, por ser nuevo usuario te regalamos el 15% de descuento. Ingresa el código NUEVOHL";
      }
    })
    .catch((error) => {
